 ...
 * @author Admin
 */
@WebServlet(name = "TyLoad", urlPatterns = {"/load"})
public class LoadType extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet HomeLoad</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet HomeLoad at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
//        ArrayList<Books> listB = ;
        BooksDAO bDAO = new BooksDAO();
        CategoryDAO cDAO = new CategoryDAO();
        PrintWriter out = response.getWriter();
        int cid = Integer.parseInt(request.getParameter("cid"));
        String cname = cDAO.getCategoryById(cid);

        ArrayList<Books> blist = bDAO.loadBooksByCategoryId(cid);
        out.print("<div class=\"relative relative-content\">\n"
                + "                            <span class=\"page-title\">"+cname+"</span>\n"
                + "                        </div>\n"
                + "                        <div class=\"items\">");
        for (Books b : blist) {
